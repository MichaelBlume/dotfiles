...
# - add cron job to keep number of shards under control
#
...
# cc-text4x external ip: 23.22.104.115
10.17.163.72	cc-test4x cc-test4x.uni.loggly.net
# cc-test8x external ip: 23.20.22.255
10.58.47.232	cc-test8x cc-test8x.uni.loggly.net

10.119.73.54	cc-hammer4x cc-hammer4x.rnd.loggly.net
10.33.69.42	cc-hammer8x cc-hammer8x.rnd.loggly.net
...
setupSolr () {
    ...
    # add cron job to expire all shards more than 20 minutes old
    #
    echo `date` ": INFO : Creating /etc/cron.d/expire"
    echo "*/5 * * * * root curl 'http://localhost:8983/solr/admin/cores?action=expire&cid=1.101.1&time=NOW-20MINUTES'" > /tmp/expire
    sudo cp /tmp/expire /etc/cron.d

    echo `date` ": INFO : Fixing Loggly.properties for beSolr"
    cd /opt/beSolr/dist/etc
    grep -v hosts Loggly.properties | grep -v 'index.shard.minutes' | grep -v 'delay.addindexer.millis' | grep -v 'index.cores.percent' | grep -v 'index.queue.maxsize' | grep -v 's3.enabled' | grep -v 'index.trackShards' | grep -v 'index.discard.alert'  > /tmp/newProps
    NOW=`date`
...
# Let Indexers spin up quicker (one new indexer every 5 seconds)
#
delay.addindexer.millis=5000

# use 1/3 of available CPU cores for indexing
#
index.cores.percent=33

# Use a smallish queue so we don't chew up all available RAM and fall behind
# when we get overloaded. Since we're deliberately going to overload its
# ok to toss events.
#
index.queue.maxsize=1000000

# turn down complaining about discrded events, since its going to happen a lot
#
index.discard.alert=1000000

# Don't use S3
#
s3.enabled=false

# Don't track shards in mongo
#
index.trackShards=false
...
EOF
    ...
    cd /opt/beSolr
    mem=`grep MemTotal /proc/meminfo  | awk '{print int($2/3)}'`
...
echo "STOPPED Solr. Collecting Metrics..."

if [ ! -e history ]
then
    sudo mkdir history
fi
sudo chmod ugo+rwx history
fgrep IdxM logs/* > history/run.ended.`date +%Y%m%d.%H%M%S`

echo "STOPPED solr, collected Metrics. You have 10 seconds to ^C before the next run starts..."
sleep 10
...
tail -F logs/beSolr.log logs/beSolr_console.log
EOF
...
setupHammer() {
    ...
    echo `date` ": INFO : Ensuring we have the latest version of zookeeper"
    sudo apt-get -y install hadoop-zookeeper-server
...
# hosts needed by this splitter in RnD cluster
#
hosts.solr=$sName
splitter.locate.hosts=$sName
...
EOF
    ...
    echo `date` ": INFO : Creating bounceAll.sh script"
    cat <<EOF > /tmp/bounceAll.sh
#!/bin/sh
#
# bounce zookeeper, collector, splitter & hammer - autogenerated by $0 on $NOW
#
echo \`date\` ": INFO : STOPPING ZooKeeper"
sudo /etc/init.d/hadoop-zookeeper-server stop

echo \`date\` ": INFO : STOPPING Collector"
cd /opt/beCollector/dist
sudo ../beCollector stop
sleep 1
sudo kill -9 \`ps -ef | grep beCollector | grep -v grep | grep -v tail | awk '{print \$2}'\`

echo \`date\` ": INFO : STOPPING beSplitter"
cd /opt/beSplitter/dist
sudo ../beSplitter stop
sleep 1
sudo kill -9 \`ps -ef | grep beSplitter | grep -v grep | grep -v tail | awk '{print \$2}'\`

echo \`date\` ": INFO : STOPPING beHammer"
cd /opt/beHammer/dist/bin
sudo kill -9 \`ps -ef | grep beHammer | grep -v grep | grep -v tail | awk '{print \$2}'\`

echo \`date\` ": INFO : STOPPED EVERYTHING - You have 10 seconds to hit ^C to avoid starting the next test"

sleep 10

echo \`date\` ": INFO : STARTING zookeeper"
cd /var/log/zookeeper
sudo rm zookeeper.log zookeeper.out
sudo /etc/init.d/hadoop-zookeeper-server start

echo \`date\` ": INFO : STARTING beCollector"
cd /opt/beCollector/dist
sudo rm beCollector.pid logs/*
sudo ../beCollector start

echo \`date\` ": INFO : STARTING beSplitter"
cd /opt/beSplitter/dist
sudo rm beSplitter.pid logs/*
sudo ../beSplitter start

sleep 1
echo \`date\` ": INFO : CLEANING Collection 1.101 from ZooKeeper "
curl "http://localhost:6983/admin?action=zknuke&zkpath=/collections/1.101"

echo \`date\` ": INFO : STARTING hammer"
cd /opt/beHammer/dist/bin
nohup java -jar beHammer.jar -logs ../zoto_sample_logs.log.100k -cid 1.101.1 -eps 30000 -run -1 -stayalive &

echo \`date\` ": INFO : TAILING logs"
echo tail -F /opt/beSplitter/dist/logs/beSplitter.log /opt/beSplitter/dist/logs/beSplitter_console.log /opt/beCollector/dist/logs/beCollector.log /opt/beCollector/dist/logs/beCollector_console.log /mnt/log/zookeeper/zookeeper.log
tail -F /opt/beSplitter/dist/logs/beSplitter.log /opt/beSplitter/dist/logs/beSplitter_console.log /opt/beCollector/dist/logs/beCollector.log /opt/beCollector/dist/logs/beCollector_console.log /mnt/log/zookeeper/zookeeper.log
EOF

    sudo cp /tmp/bounceAll.sh /opt/bounceAll.sh
    sudo chmod ugo+x /opt/bounceAll.sh
    
    echo `date` ": INFO : Fixing zookeeper to run in standalone mode"
    cat <<EOF > /tmp/zoo.cfg
# zookeeper config - autogenerated by $0 on $NOW
#
...
EOF
    sudo cp /tmp/zoo.cfg /etc/zookeeper/zoo.cfg
    sudo cp /etc/zookeeper/conf_example/log4j.properties /etc/zookeeper
...
case $HOSTNAME in
    ...
    cc-test*)
	myNum=`echo $HOSTNAME | cut -c8,9`
	hName="cc-hammer$myNum"
	sName="cc-test$myNum"
	setupSolr "$hName" "$sName"
	;;
    ...
    cc-hammer*)
	myNum=`echo $HOSTNAME | cut -c10,11`
	hName="cc-hammer$myNum"
	sName="cc-test$myNum"
	setupHammer "$hName" "$sName"
	;;
